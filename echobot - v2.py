...
global chat_list,user_list,run_server,adminchatid
...
lang_opts = ['English', '简体中文','繁體中文','हिंदी','தமிழ்','বাংলা','Filipino','Indonesian', 'Malay',\
    'မြန်မာ','ไทย','Việt Nam','日本語','한국의', 'Nederlands','Français','Deutsch','Italiano','Español']
    ...
    def on_chat_message(self, msg):
        ...
        if content_type == 'text':
            ...
            try:
                ...
                txt = process_voice(fn, self.audio_lang)
                resp = "Sorry for not being able to recognise this audo" if txt=="" else txt
            except:
                pass
        ...
        if resp=='/end':
            ...
            user_list[chat_id] = [self.username, self.lang, self.audio_lang ]
        ...
        ## trigger when live chat is initiated
            ...
            if resp in lang_opts:
                ...
                self.audio_lang = lang_audio[n]
                txt = f"You had selected {resp} language"
                user_list[chat_id][1] = self.lang
                user_list[chat_id][2] = self.audio_lang
                print(self.lang,self.audio_lang)
...
def process_voice(fn, lang="en-US"):
    try:
...
def do_main():
    ...
    chat_list = {}
    err = 0    
    #z = os.environ['ADMINCHATID']
    z=71354936
    adminchatid=int(z)
    EchoBotToken = os.environ['KOPIBOT']
    #EchoBotToken = "812577272:AAEgRcGYOGzkN9AoJQKLusspiowlUuGrtj0"  # @vmdevbot
