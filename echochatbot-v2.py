...
option_qr =  "QR Code"
option_genpw = "Generate Password"
echobot_menu = [[option_chat, option_lang, option_text2voice, option_voice2text],[option_face, option_qr, option_genpw, option_back]]
option_cv2 = "Train Image Model"
option_img = "Image Detection"
face_menu = [[option_cv2, option_img , option_back]]
option_qrgen = "Generate QRcode"
option_qrdecode = "Read QRcode"
qr_menu = [[option_qrgen, option_qrdecode , option_back]]
...
class BotInstance():
    def __init__(self, Token):
        ...
        self.define_keys(qr_menu, self.keys_dict[option_qr])
    ...
    def on_chat_message(self, msg):
        ...
        if content_type == 'text':
            ...
            if ftype=="application/pdf":
                ...
                if self.menu_id == keys_dict[option_qrdecode]:
                    read_qr = True    
                else:
                    img2txt = True
            ...
            if self.menu_id == keys_dict[option_img] :
                image_det = True
            elif self.menu_id == keys_dict[option_qrdecode]:
                read_qr = True
            else:
                img2txt = True
        ...
        if read_qr:
            fname = get_attachment(bot, fid)
            txt=(decode(Image.open(fname)))[0].data.decode()
        ...
        if resp=='/end':
        ...
        elif resp.startswith("/qr"):
            qrcode = resp[4:]
            gen_qrcode(bot, chat_id, qrcode)
            ...
            if chat_id in self.parentbot.chat_list:
                ...
                txt = "This section is for QR code."
                bot_prompt(bot, chat_id, txt, qr_menu)
                self.menu_id = keys_dict[option_qr]             
            elif resp == option_genpw :
                genpw(bot, chat_id)
        ...
        elif self.menu_id == keys_dict[option_qr] :
            if resp == option_back :
                bot_prompt(bot, chat_id, "You are back in the main menu", self.mainmenu)
                self.menu_id = keys_dict[option_mainmenu]
            elif resp == option_qrgen :
                txt = "Please enter the text or URL for QR code generation."
                bot.sendMessage(chat_id, txt)
                self.menu_id = keys_dict[option_qrgen]
            elif resp == option_qrdecode :
                txt = "Please upload a photo of the QR code."
                bot.sendMessage(chat_id, txt)
                self.menu_id = keys_dict[option_qrdecode]

        elif self.menu_id == keys_dict[option_qrgen] :
            if resp == option_back :
                bot_prompt(bot, chat_id, "You are back in the main menu", self.mainmenu)
                self.menu_id = keys_dict[option_mainmenu]
            else:
                gen_qrcode(bot, chat_id, resp)
                bot_prompt(bot, chat_id, "You are back in the main menu", self.mainmenu)
                self.menu_id = keys_dict[option_mainmenu]
...
def genpw(bot, chat_id):
    psw = "".join(random.sample(list(string.ascii_lowercase)+list(string.ascii_uppercase)+list(string.digits)+list(string.punctuation),8))
    result = "The generated password is\n<pre>" + psw + "</pre>"
    bot.sendMessage(chat_id,result,parse_mode='HTML')
    return

def gen_qrcode(bot, chat_id, resp):
    qrcode = str(resp).strip()
    if qrcode == "":
        return
    fname = "qrcode.png"
    img = qr.make(qrcode)
...
def on_callback_query(msg):
    ...
    recipient = msg['message']['chat']['id']
    if query_data=='About':
        txt = "About developer\nKH is a telegram developer focusing on business solution with AI\nContact @kimhuat"
        echobot.bot.answerCallbackQuery(query_id, txt, show_alert=True)
    elif query_data=='Close':
        edited = echobot.edited
        if edited != 0:
            echobot.bot.deleteMessage(edited)
...
def on_chat_message(msg):
    ...
    if message[0] == '/':
        cmd = message.lower()
        if cmd == '/start':
            yn_keyboard=[list(map(lambda c: InlineKeyboardButton(text=c, callback_data=c), ['About','Close']))]
            qn_keyboard = InlineKeyboardMarkup(inline_keyboard=yn_keyboard)
            helptext = "Hi, the echochatbot has reached the end of support, thanks for trying out the bot."
            sent = bot.sendMessage(chat_id, helptext, reply_markup=qn_keyboard)
            echobot.edited = telepot.message_identifier(sent)
