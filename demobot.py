...
global vmbot, ft_model, dt_model, mcq_analysis, rdscon, rds_connstr
...
faculty_menu = [[fc_student, fc_intv, fc_notf, option_back]]
...
class BotInstance():
    def __init__(self):
        ...
        self.client_name = client_name = "Demo"
...
class MessageCounter(telepot.helper.ChatHandler):    
    ...
    def find_course(self, stud):
        ...
        if stud==0:
            ...
            qry += f"and c.enabled=1 AND a.client_name='{client_name}' and {sub_str}(course_id,-4)='{yrnow}' ORDER BY a.course_id;"
            df = rds_df(qry)
            if df is not None:
                df.columns = ['course_id','course_name']
                course_id_list = [x for x in df.course_id]
...
def load_progress(df, student_id, vars, client_name, resp_dict, pass_rate, stagedf):
    ...
    missing_dates = ['' for x in stg_list]
    missing_stages = []
    stagebyprogress = ""
    statusbyprogress = ""
    mcnt = 0
...
def analyze_cohort(client_name, course_id, bot, chat_id):
    ...
    avgsum_list = count_avg_cols(client_name, course_id, 13, "mcq_avg")
    ...
    avgsum_list = count_avg_cols(client_name, course_id, 13, "as_avg")
    rr = [ 1 if r>0 else 0 for r in avgsum_list ]
    rsum = sum(rr)    
    idx = 0
    if rsum>0:        
